...
elasticStretchingForcePlate::elasticStretchingForcePlate(elasticPlate &m_plate, timeStepperPlate &m_stepper)
{
	plate = &m_plate;
	stepper = &m_stepper;
...
elasticStretchingForcePlate::~elasticStretchingForcePlate()
{
	;
}

void elasticStretchingForcePlate::computeFs()
{
...
void elasticStretchingForcePlate::computeJs()
{
	for (int i = 0; i < plate->edgeNum; i++)
	{
		arrayNum = plate->v_edgeElement[i].arrayNum;
...
void elasticStretchingForcePlate::setFirstJacobian()
{
	for (int i = 0; i < plate->edgeNum; i++)
	{
		arrayNum = plate->v_edgeElement[i].arrayNum;
