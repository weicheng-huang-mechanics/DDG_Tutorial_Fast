...
void display(void)
{
	while ( myWorld.simulationRunning() > 0)
	{
		...
		*/
		...
		glColor3f(1.0, 0.0, 0.0);
		glLineWidth(5.0);
		double boxSize = myWorld.getBoxSize();
		...
		*/
		...
		glColor3f(1.0, 0.0, 0.0);
		glutSolidSphere(myWorld.getBoxSize(), 10, 10);
		glTranslatef(-xCurrent(0), -xCurrent(1), -xCurrent(2));
		...
		glBegin(GL_LINES);
		for (int i = 0; i < myWorld.getBoundaryEdge(); i++)
		{
		...
		glBegin(GL_LINES);
		for (int i = myWorld.getBoundaryEdge(); i < NPTS; i++)
		{
			Vector3d xCurrent_1 = myWorld.getScaledCoordinate(i, 0);
			Vector3d xCurrent_2 = myWorld.getScaledCoordinate(i, 1);
			glVertex3f(xCurrent_1(0), xCurrent_1(1), xCurrent_1(2));
